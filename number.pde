...
void draw(){
  ...
  for(var i = 9; i>=0;i--){
    ...
    fill(0);
    text(String.fromCharCode(i+48),letters[i][0]+20,letters[i][1]+20);
    if(letters[i][2]){
      ...
      //if(letters[i][2]&&select===2){
            ...
            if(i>0&&dist((letters[i][0]-w)/1.2,letters[i][1],(letters[i-1][0])/1.2,letters[i-1][1])<w*0.6){
              ...
              stars[stars.length] = [random(100,1100),random(100,600),100,millis()];
              letters[i-1][5]=true;
            ...
            if(i<9&&dist((letters[i][0]+w)/1.2,letters[i][1],(letters[i+1][0])/1.2,letters[i+1][1])<w*0.6){
              if(letters[i+1][5]===false){
              stars[stars.length] = [random(100,1100),random(100,600),100,millis()];
              letters[i+1][5]=true;
